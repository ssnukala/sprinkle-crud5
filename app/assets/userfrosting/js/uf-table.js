...
(function ($, window, document, undefined) {
  ...
  function Plugin(element, options) {
    ...
    var dataAttributeDefaults = {
      info: {
        messageEmptyRows: infoContainer.data("message-empty-rows") ? infoContainer.data("message-empty-rows") : "Sorry, we've got nothing here.",
      },
      tablesorter: {
        widgetOptions: {
          // possible variables: {size}, {page}, {totalPages}, {filteredPages}, {startRow}, {endRow}, {filteredRows} and {totalRows}
          // also {page:input} & {startRow:input} will add a modifiable input in place of the value
          pager_output: pagerContainer.data("output-template") ? pagerContainer.data("output-template") : "{startRow} to {endRow} of {filteredRows} ({totalRows})", // default if not set on data-* attribute
        },
      },
    };
    ...
    if (this.settings.useLoadingTransition) {
      ...
      tableElement
        .bind("sortStart filterStart pageMoved", function () {
    ...
    for (var col = 0; col < columns.length; col++) {
      ...
      if (typeof columnTemplate === "string") {
        this.columnTemplatesIndexed.push({ colName: columnName, template: Handlebars.compile($(columnTemplate).html()) });

        //this.columnTemplatesIndexed.push(Handlebars.compile($(columnTemplate).html()));
      } else {
        this.columnTemplatesIndexed.push({ colName: columnName, template: columnTemplate });
    ...
    $(this.settings.tablesorter.widgetOptions.columnSelector_container)
      .find("label")
      .on("click", function (e) {
        e.stopPropagation();
      });
    ...
    if (data) {
      ...
      for (var row = 0; row < size; row++) {
        ...
        for (var col = 0; col < this.columnTemplatesIndexed.length; col++) {
          cellData.col = this.columnTemplatesIndexed[col].colName;
          rows += this.columnTemplatesIndexed[col].template(cellData);
        }
